...
def make_figures(values: dict[str, object], attacks: list[dict[str, object]]) -> None:
    ...
    fig, ax = plt.subplots(figsize=(9.0, max(5.2, 0.28 * len(labels))))
    ax.barh(labels, values, color="#4f8a5f")
    ax.set_xlabel("attack rounds")
...
def write_latex_tables(scorecard: list[dict[str, str]], attacks: list[dict[str, object]]) -> None:
    score_lines = [
        ...
        r"\begin{tabular}{@{}p{0.07\linewidth}p{0.22\linewidth}p{0.14\linewidth}p{0.27\linewidth}p{0.22\linewidth}@{}}",
        r"\toprule",
    ...
    for start in range(0, len(attacks), 10):
        chunk = attacks[start : start + 10]
        label = r"\label{tab:v3-attack-ledger}" if start == 0 else ""
        attack_lines.extend(
            [
                r"\begin{table}[H]",
                r"\centering",
                ...
                rf"\caption{{Fifty-round v3 self-attack ledger, rounds {start + 1}--{start + len(chunk)}. LIM rows are scoped limitations.}}{label}",
                r"\begin{tabular}{@{}p{0.04\linewidth}p{0.12\linewidth}p{0.30\linewidth}p{0.39\linewidth}p{0.06\linewidth}@{}}",
                r"\toprule",
        ...
        for row in chunk:
            status = "LIM" if row["status"] == "bounded" else str(row["status"]).upper()
            attack_lines.append(
                " & ".join(
                    [
...
def write_artifact_latex_tables(values: dict[str, object]) -> None:
    ...
    for start in range(0, len(sorted_metrics), 14):
        chunk = sorted_metrics[start : start + 14]
        label = r"\label{tab:v3-full-curves}" if start == 0 else ""
        lines.extend(
            [
                r"\begin{table}[H]",
                r"\centering",
                ...
                rf"\caption{{Full selected-tail curves for the controlled tokenized candidate pools, rows {start + 1}--{start + len(chunk)}.}}{label}",
    ...
    tail_lines.extend([r"\bottomrule", r"\end{tabular}", r"\end{table}", ""])
    ...
    bottleneck_lines.extend([r"\bottomrule", r"\end{tabular}", r"\end{table}", ""])
