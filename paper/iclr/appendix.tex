...
V4 protocol and rubric audit & \path{results/v4_cached_evidence_summary.json}; \path{results/v4_tokenized_submission_scorecard.csv}; \path{results/v4_protocol_freeze_gates.csv}; \path{results/v4_iclr_style_rubric_map.csv}; \path{results/v4_reviewer_attack_ledger.csv} \\
\bottomrule
\end{tabular}
\end{table}
...
\section{Self-Attack Ledgers}
\label{app:attack}

\paragraph{V3 base ledger.}
\input{v3_attack_ledger_table.tex}

\paragraph{V4 final ledger.}
The v4 ledger extends the base attacks to citation surface, benchmark-boundary, protocol-freeze, rubric, source-firewall, and fresh-session reproducibility failures.

\input{v4_attack_ledger_table.tex}

\section{Final V4 Acceptance Checklist}
\label{app:v4-final-checklist}

\begin{itemize}
\item The final PDF must have at least 25 pages.
\item The repo final PDF and Desktop PDF must match by SHA-256.
\item The Desktop file must be \mbox{\texttt{tokenized world model-v4.pdf}}.
\item The old Desktop v3 and v2 files must be absent.
\item \texttt{PAPER\_SOURCE\_MAP.md} must map the v4 Desktop filename to:
\begin{quote}
\ttfamily
C:/Users/wangz/tokenized world model\\
Jason-Wang313/tokenized-world-model
\end{quote}
\item The v4 self-attack ledger must contain exactly 60 rows.
\item The v4 protocol gates must pass 12/12, and the rubric map must pass 7/7.
