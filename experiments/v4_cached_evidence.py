...
def write_latex_tables(scorecard: list[dict[str, Any]], protocol: list[dict[str, Any]], rubric: list[dict[str, Any]], attacks: list[dict[str, Any]]) -> None:
    def fixed_table(path: Path, caption: str, label: str, cols: list[tuple[str, str]], rows: list[dict[str, Any]]) -> None:
    ...
    fixed_table(
        ...
        "tab:v4-scorecard",
        [("Axis", f"{p}{{0.17\\linewidth}}"), ("Gate", f"{p}{{0.08\\linewidth}}"), ("Observed", f"{p}{{0.28\\linewidth}}"), ("Artifact", f"{p}{{0.19\\linewidth}}"), ("Use", f"{p}{{0.20\\linewidth}}")],
        scorecard,
    )
    fixed_table(
        ...
        "tab:v4-protocol",
        [("Gate", f"{p}{{0.23\\linewidth}}"), ("Status", f"{p}{{0.09\\linewidth}}"), ("Evidence", f"{p}{{0.64\\linewidth}}")],
        protocol,
    )
    fixed_table(
        ...
        "tab:v4-rubric",
        [("Rubric axis", f"{p}{{0.21\\linewidth}}"), ("Status", f"{p}{{0.09\\linewidth}}"), ("Evidence", f"{p}{{0.66\\linewidth}}")],
        rubric,
    )
    attack_lines = ["% Auto-generated by experiments/v4_cached_evidence.py"]
    for start in range(0, len(attacks), 10):
        chunk = attacks[start : start + 10]
        label = r"\label{tab:v4-attack-ledger}" if start == 0 else ""
        attack_lines.extend(
            [
                r"\begin{table}[H]",
                r"\centering",
                ...
                rf"\caption{{Full 60-round v4 reviewer attack ledger, rounds {start + 1}--{start + len(chunk)}.}}{label}",
                r"\begin{tabular}{@{}p{0.04\linewidth}p{0.12\linewidth}p{0.31\linewidth}p{0.38\linewidth}p{0.05\linewidth}@{}}",
                r"\toprule",
